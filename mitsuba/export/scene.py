...
class SceneExporter:
	...
	def writeHeader(self):
		...
		self.openElement('scene',{'version' : '0.6.0'})
		return True
	...
	def parameterNoName(self, paramType, attributes = {}):
		self.out.write('\t' * self.indent + '<%s' % (paramType))
		for (k, v) in attributes.items():
			self.out.write(' %s=\"%s\"' % (k, v))
		self.out.write('/>\n')
	...
	def exportLamp(self, scene, lamp):
		...
		if ltype == 'POINT':
			if lamp.data.mitsuba_lamp.pointLight == False:
				self.openElement('shape', { 'type' : 'sphere'})
				#self.exportPoint(lamp.location)
				self.exportMotionData(scene, lamp)
				self.parameter('float', 'radius', {'value' : lamp.data.mitsuba_lamp.radius})			#7*
				...
				self.openElement('emitter', { 'type' : 'point' , 'id' : '%s-light' % name})
				#self.exportPoint(lamp.location)
				#self.parameter('point', 'position', {'x' : lamp.location[0],'y' : lamp.location[1],'z' : lamp.location[2]})
				self.exportMotionData(scene, lamp)
				self.parameter('spectrum', 'intensity', {'value' : "%f %f %f" % (lamp.data.color.r*mult, lamp.data.color.g*mult, lamp.data.color.b*mult)})
				self.closeElement()
		elif ltype == 'AREA':
	...
	def exportMotionData(self, scene, lamp):
		#if len(lamp.animation_data.action.fcurves) != 0 and len(lamp.animation_data.action.fcurves.keyframe_points) != 0:
		anim = lamp.animation_data
		if anim is not None and anim.action is not None and len(anim.action.fcurves) != 0:
		#if True:
			frameStart = scene.frame_start
			frameEnd = scene.frame_end

			self.openElement('animation', {'name' : 'toWorld'})
			...
			for k in f.keyframe_points:
				...
				self.openElement('transform', {'time' : localTime})
				self.parameterNoName('translate', {'x' : pos[0],'y' : pos[1],'z' : pos[2]})
				self.closeElement()
	...
	def exportCamera(self, scene, camera):
		...
		if mcam.useMotionBlur == True:
			self.parameter('float', 'shutterOpen', {'value' : str(mcam.timeStart)})
			self.parameter('float', 'shutterClose', {'value' : str(mcam.timeEnd)})
